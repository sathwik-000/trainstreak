...
async function streaks(uid) {
  ...
  const workouts = new Set(wr.rows.map(x => String(x.date).slice(0, 10)));
  const rests = new Set(rr.rows.map(x => String(x.date).slice(0, 10)));
  const t = today();

  // A workout advances the streak. A planned/recorded rest day is neutral and
  // does not break the streak. A day that was scheduled for a workout but has
  // neither a workout nor a rest-day entry breaks the streak. Unscheduled days
  // are neutral as well.
  const dayState = (k) => {
    if (workouts.has(k)) return 'workout';
    if (rests.has(k)) return 'rest';
    return sr[mondayIndex(k)] === false ? 'unscheduled' : 'missed';
  };
  ...
  for (let i = 0; i < 5000; i++) {
    ...
    if (state === 'workout') current++;
    k = addDays(k, -1);
  }

  // Longest streak: count workout days while allowing neutral rest and
  // unscheduled days between them; reset on a missed scheduled workout day.
  let longest = 0;
  let run = 0;
  let c = addDays(t, -4999);
  for (let i = 0; i < 5000; i++) {
    const state = dayState(c);
    if (state === 'missed') {
      run = 0;
